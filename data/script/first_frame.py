...
def extract_first_token(dataset_path, detector_path, dataset_name='NuScenes', dataset_version='trainval'):
    ...
    if dataset_name == 'NuScenes':
        ...
        # write token table (filename uses val/test; folder still trainval/test)
        os.makedirs(FIRST_TOKEN_ROOT_PATH + dataset_version, exist_ok=True)
        split_tag = 'val' if dataset_version == 'trainval' else dataset_version
        FIRST_TOKEN_PATH = (
            FIRST_TOKEN_ROOT_PATH
            + dataset_version
            + f"/nuscenes_{split_tag}_first_sample_tokens.json"
        )

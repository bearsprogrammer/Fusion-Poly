...
from motion_module.kalman_filter import LinearKalmanFilter, ExtendKalmanFilter, IEKF
...
class Trajectory:
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict):
        ...
        assert KF_type in ['LinearKalmanFilter', 'ExtendKalmanFilter', 'IEKF'], "must use specific kalman filter"
    ...
    def state_predict(self, timestamp: int, is_key_frame: bool) -> None:
        """
        predict trajectory's state
        :param timestamp: current frame id
        """
        self.timestamp = timestamp
        self.life_management.predict(timestamp, is_key_frame) # whether or not diverse high and low frequency, need to be considered
        self.motion_management.predict(timestamp, is_key_frame)
        if self.cfg['geometry_model']['use'][self.class_label]:
            self.geometry_management.predict(timestamp, self.motion_management[timestamp])
        self.score_management.predict(timestamp, is_key_frame, self.motion_management[timestamp])

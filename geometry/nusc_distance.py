...
def fast_giou_3d_unique(boxes_a: dict, boxes_b: dict) -> Tuple[np.array, np.array]:
    """
    fast implementation of 3d giou.
    'np_dets': np.array, [det_num, 14](x, y, z, w, l, h, vx, vy, ry(orientation, 1x4), det_score, class_label)
    'np_dets_norm_corners': np.array, [det_num, 4 (top, left, bottom, right in the global frame)]
    ...
    infos_a, infos_b = boxes_a['np_dets'], boxes_b['np_dets']  # [box_num, 14]
    norm_bcs_a, norm_bcs_b = boxes_a['np_dets_norm_corners'], boxes_b['np_dets_norm_corners']  # [box_num, 4]
    assert len(infos_a) == len(infos_b), 'the numbers of boxes_a and boxes_b must be equal in fast giou 3d unique'
    ...
    # process norm bottom corners, size and center for parallel computing
    rep_norm_bcs_a, rep_norm_bcs_b = norm_bcs_a, norm_bcs_b # [num, 4]
    za, zb, ha, hb = infos_a[:, [2]], infos_b[:, [2]], infos_a[:, [5]], infos_b[:, [5]]  # [num, 1]
    bboxa_area = (rep_norm_bcs_a[:, [2]] - rep_norm_bcs_a[:, [0]]) * (rep_norm_bcs_a[:, [3]] - rep_norm_bcs_a[:, [1]])
    bboxb_area = (rep_norm_bcs_b[:, [2]] - rep_norm_bcs_b[:, [0]]) * (rep_norm_bcs_b[:, [3]] - rep_norm_bcs_b[:, [1]])
...
def fast_iou_bev(boxes_a: dict, boxes_b: dict) -> np.array:
    """
    Parallel implementation of fast bev iou.
    'np_dets': np.array, [det_num, 14](x, y, z, w, l, h, vx, vy, ry(orientation, 1x4), det_score, class_label)
    'np_dets_norm_corners': np.array, [det_num, 4 (top, left, bottom, right in the global frame)]
    :param boxes_a: dict, a collection of NuscBox info, keys must contain 'np_dets' and 'np_dets_norm_corners'
    :param boxes_b: dict, a collection of NuscBox info, keys must contain 'np_dets' and 'np_dets_norm_corners'
...
def iou_2d(bboxs_a, bboxs_b) -> np.array:
    """parallel implementation of iou 2d on the image plane
    'np_dets': np.array, [det_num, 14](x, y, z, w, l, h, vx, vy, ry(orientation, 1x4), det_score, class_label)
    'np_dets_bbox2d': np.array, [det_num, 4], (x0, y0(top-left), x1, y1(bottom-right)), reproject bbox 2d
    
    Args:
        bboxs_a (dict): dict, a collection of NuscBox info, keys must contain 'np_dets' and 'np_dets_bbox2d'
        bboxs_b (dict): dict, a collection of NuscBox info, keys must contain 'np_dets' and 'np_dets_bbox2d'

    Returns:
        np.array: [det_num, tra_num], 2d iou between two bboxes collections
    """
    # corner case, 1d array(collection only has one box) to 2d array
    if bboxs_a.ndim == 1: bboxs_a = bboxs_a[None, :]
    if bboxs_b.ndim == 1: bboxs_b = bboxs_b[None, :]
    assert bboxs_a.shape[1] == 6 and bboxs_b.shape[1] == 6, "dim must be 6"
    
    # load info
    corners_a, corners_b = bboxs_a[:, 0:4], bboxs_b[:, 0:4]
    
    # mask matrix, True denotes different(invalid), False denotes same(valid)
    bool_mask, seq_mask = mask_between_boxes(bboxs_a[:, -1], bboxs_b[:, -1])
    bool_mask, seq_mask = logical_or_mask(bool_mask, seq_mask, bboxs_a, bboxs_b)
    
    # expend dims, [det_num, tra_num, 4(x0, y0, x1, y1)]
    corners_a = corners_a[:, None, :].repeat(len(corners_b), 1)
    corners_b = corners_b[None, :, :].repeat(len(corners_a), 0)
    
    # calu inter area, [det_num, tra_num]
    min_x = np.minimum(corners_a[:, :, 2], corners_b[:, :, 2])
    max_x = np.maximum(corners_a[:, :, 0], corners_b[:, :, 0])
    min_y = np.minimum(corners_a[:, :, 3], corners_b[:, :, 3])
    max_y = np.maximum(corners_a[:, :, 1], corners_b[:, :, 1])
    
    inter_x = np.maximum(np.zeros_like(min_x), (min_x - max_x))
    inter_y = np.maximum(np.zeros_like(min_y), (min_y - max_y))
    inter_area = inter_x * inter_y
    
    # calu union area
    bboxa_area = (corners_a[:, :, 2] - corners_a[:, :, 0]) * (corners_a[:, :, 3] - corners_a[:, :, 1])
    bboxb_area = (corners_b[:, :, 2] - corners_b[:, :, 0]) * (corners_b[:, :, 3] - corners_b[:, :, 1])
    union_area = bboxa_area + bboxb_area - inter_area
    
    # mask invalid value
    iou2d = inter_area / union_area
    iou2d[bool_mask] = -1
    
    return iou2d, None


def giou_2d(bboxs_a, bboxs_b) -> np.array:
    """parallel implementation of giou 2d on the image plane
    'np_dets': np.array, [det_num, 14](x, y, z, w, l, h, vx, vy, ry(orientation, 1x4), det_score, class_label)
    'np_dets_bbox2d': np.array, [det_num, 4], (x0, y0(top-left), x1, y1(bottom-right)), reproject bbox 2d
    ...
    # calu convex area
    union_x = np.maximum(corners_a[:, :, 2], corners_b[:, :, 2]) - np.minimum(corners_a[:, :, 0], corners_b[:, :, 0])
    union_y = np.maximum(corners_a[:, :, 3], corners_b[:, :, 3]) - np.minimum(corners_a[:, :, 1], corners_b[:, :, 1])
    convex_area = union_x * union_y

    # mask invalid value
    giou2d = inter_area / union_area - (convex_area - union_area) / convex_area
    giou2d[bool_mask] = -1
...
#     xyz_a1, xyz_b1 = expand_dims(infos_a[:, 0:3], len(infos_b), 1), expand_dims(infos_b[:, 0:3], len(infos_a), 0)

#     deta = xyz_a1 - xyz_b1

#     elementwise_norms = np.linalg.norm(deta, ord=2, axis=2)

#     eucl_dis = elementwise_norms
#     eucl_dis[bool_mask] = np.inf
